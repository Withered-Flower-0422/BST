...
- Put the `_Typings` folder and `tsconfig.json` file in `%USERPROFILE%\AppData\LocalLow\Mushreb\BME Pro HDRP\Assets\Scripts`.
...
- If you don't want type checking during coding:
    ...
    - Ignore every file: change the `checkJS` setting in `tsconfig.json` to `false`.
